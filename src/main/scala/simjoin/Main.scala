...
object Main {
  def main(args: Array[String]) {     
    ...
    val resultSizeCart = cartesian.count
    println(resultSizeCart)
    val t2Cartesian = System.nanoTime
    println("2: " + (t2Cartesian-t1Cartesian)/(Math.pow(10,9)))

    /*val testSet = cartesian.collect().toList.map(x => (x._1._1, x._2._1)) filterNot res.collect().toList.map(x => (x._1, x._2)).toSet
    println("TEST: " + testSet)*/
    //println(resultSize == resultSizeCart)
